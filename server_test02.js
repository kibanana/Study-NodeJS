...
const server = http.createServer((request, response) => {  // 서버 만드는 메소드
    ...
    if (request.method === 'GET') { // GET 요청이면
      if (path === '/about') { // 주소가 /about이면
        ...
        fs.readFile(__dirname + '/test02_about.html', (err, data) => { // 파일 읽는 메소드
          if (err) {
        ...
        fs.readFile(__dirname + '/test02_main.html', (err, data) => {
          if (err) {
