...
//     if(beforeHungry.age > afterHungry.age) return 1;
// 나이 오름차순
//     else if(beforeHungry.age < afterHungry.age) return -1;
//     else if(beforeHungry.name > afterHungry.name) return -1;
// 반환문을 수정하기보다는 조건문을 수정함으로써 오름차순/내림차순 조정
//     else if(beforeHungry.name < afterHungry.name) return 1;
// 이름 내림차순(z~A)
//     return 0;
// });
...
// 092
for (let j = 0; j < capital.length; j += 1) {
  if (capital[j] === 'Seoul') {
